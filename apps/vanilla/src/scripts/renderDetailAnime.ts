...
function renderDetailAnime(anime: AnimeDetail | null): void {
  ...
  const htmlStudiosList = anime.studios.map(studio => `
      <h2>${studio.name}</h2>
  `).join('');

  const htmlGenreList = anime.genres.map(genre => `
      <h2>${genre.name}</h2>
  `).join('');
  ...
  detailWrapper.innerHTML += `
    ...
    <div class="detail__trailer">
      <button class ="button button_trailer" type="button">Watch trailer</button>
    </div>
    <div class="detail__content">
      <h2 class="detail__title">${anime.titleEnglish}</h2>
      <h2 class="detail__title">${anime.titleJapan}</h2>
  ...
  const buttonTrailer = detailWrapper.querySelector<HTMLButtonElement>('.button_trailer');
  if (imageElement == null || buttonTrailer == null) {
    return;
  }
